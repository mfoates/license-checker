    ...
    files.forEach(function(filename, index) {
        ...
        if (fs.lstatSync(licenseFile).isFile()) {
            ...
            if (index === 0) {
                ...
                if (include("licenseText") && ( options.customFormat || options.templatePath ) ) {
                    if (!content) {
                        content = fs.readFileSync(licenseFile, { encoding: 'utf8' });
                    }
                    /*istanbul ignore else*/
                    if (options.templatePath) {
                        moduleInfo.licenseText = content.replace(/^(.*)$/mg, '> $1').trim();
                    } else if (options._args && !options._args.csv) {
                        moduleInfo.licenseText = content.trim();
                    } else {
                        moduleInfo.licenseText = content.replace(/"/g, '\'').replace(/\r?\n|\r/g, " ").trim();
                    }
                    if (options._args && options._args.addMarkdownBlock) {
                        moduleInfo.licenseText = moduleInfo.licenseText.replace(/^(.*)$/mg, '> $1').trim();
                    }
                    if (options._args && options._args.escapeJinja) {
                        moduleInfo.licenseText = moduleInfo.licenseText.replace(/{{/g, "&#123;&#123;").replace(/}}/g, "&#125;&#125;");
                    }
                    if (options._args && options._args.escapeAnsibleRaw) {
                        moduleInfo.licenseText = '{% raw %}' + moduleInfo.licenseText + '{% endraw %}';
                    }
                    if (options._args && options._args.encodeUtf8) {
                        moduleInfo.licenseText = encodeURIComponent(moduleInfo.licenseText);
                    }
                }

                if(include('copyright') && ( options.customFormat || options.templatePath ) ) {
                    if (!content) {
                        content = fs.readFileSync(licenseFile, { encoding: 'utf8' });
                    }
...
var parseTemplate = function(templatePath) {
    if (typeof templatePath !== 'string') {
        return new Error('did not specify a template path');
    }
...
/**
* Exports data according to template file which has EJS syntax (https://ejs.co).
* @method
* @param  {JSON} licenseData       The sorted JSON data from all packages.
* @param  {String} templatePath    The EJS template file for creating the output
* @return {String}                 The returning plain text.
*/
exports.asTemplate = function(licenseData, templatePath) {

    var template = parseTemplate(templatePath);

    var text = ejs.render(template, { licenses: licenseData });
